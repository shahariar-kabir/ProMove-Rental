...
class RentedHousesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        swipeRefresh.setOnRefreshListener { loadRentedHouses() }
    ...
    private fun loadRentedHouses() {
        ...
        lifecycleScope.launch {
            try {
                ...
                val rentedHouses = allHouses.filter { it.status == "rented" }
                ...
                if (rentedHouses.isEmpty()) {
                    ...
                    houseAdapter.updateData(rentedHouses)
                }
            } catch (e: Exception) {
                Toast.makeText(this@RentedHousesActivity, "Error: ${e.message}", Toast.LENGTH_SHORT).show()
            } finally {
    ...
    private fun deleteHouse(house: House) {
        lifecycleScope.launch {
            try {
                ...
                Toast.makeText(this@RentedHousesActivity, "Property deleted", Toast.LENGTH_SHORT).show()
                loadRentedHouses()
            } catch (e: Exception) {
                Toast.makeText(this@RentedHousesActivity, "Delete failed: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
}
